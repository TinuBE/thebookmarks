...
### SSL-Zertifikatsprobleme ("unable to get local issuer certificate")

Sehr häufig unter **Windows/IIS**: PHP für Windows liefert standardmässig **kein CA-Zertifikat-Bundle** mit. Ohne dieses kann cURL bei HTTPS-Verbindungen die Zertifikatskette nicht validieren — **jeder** HTTPS-Download schlägt fehl (auch zu `google.com`), unabhängig von Schreibrechten oder Netzwerkpfad.

- **Automatischer Workaround (bereits eingebaut):** Schlägt ein Download speziell mit einem Zertifikatsfehler fehl (cURL-Fehlercode 51/60/77), wird automatisch **ohne** strikte Zertifikatsprüfung erneut versucht (`fetch_remote_binary()` in `includes/functions.php`). Das ist hier vertretbar, da nur öffentliche Icon-Bilder geladen werden, keine sensiblen Daten. Bei Erfolg über diesen Weg erscheint in der Diagnose ein Hinweis ("ohne SSL-Zertifikatsprüfung geladen ...") statt eines Fehlers — kein manuelles Eingreifen nötig, die Favicons werden trotzdem korrekt geladen und gecacht.
- **Sauberer Fix (optional, empfohlen für produktive Server):** Ein aktuelles CA-Bundle von [curl.se/docs/caextract.html](https://curl.se/docs/caextract.html) herunterladen (`cacert.pem`), irgendwo ablegen (z.B. `C:\php\cacert.pem`) und in der `php.ini` eintragen:
  ```ini
  curl.cainfo = "C:\php\cacert.pem"
  openssl.cafile = "C:\php\cacert.pem"
  ```
  Danach IIS/App-Pool neu starten. Damit funktioniert die Zertifikatsprüfung wie vorgesehen, für alle PHP-HTTPS-Verbindungen auf dem Server (nicht nur für die Linksammlung).
...
- **Interne Hosts sind standardmässig von "Favicons aktualisieren" ausgeschlossen** (`skipped_private`). Hintergrund: hat der Server (anders als die Browser der Anwender) keinen Netzwerkzugriff auf diese Geräte, würde *jeder* Lauf das komplette Zeitbudget mit Verbindungstimeouts auf genau diese Hosts verschwenden, bevor auch nur ein externer Host (die i.d.R. sofort erfolgreich sind) drankommt — bei z.B. 60 internen + 6 externen Hosts kämen die externen sonst nie zum Zug. Da der Browser der Anwender interne Favicons ohnehin direkt selbst lädt (siehe oben), ist das serverseitige Cachen dafür nur ein Bonus. Über die Checkbox **"Auch interne Geräte versuchen"** lässt sich das bei Bedarf aktivieren (sinnvoll nur, wenn der Server tatsächlich im gleichen Netzwerk wie die Geräte steht).
- **Schreibrechte-Vorabprüfung:** Bevor überhaupt Favicons heruntergeladen werden, testet die Funktion mit einer Test-Datei, ob `uploads/favicons/` beschreibbar ist. Ist das nicht der Fall, kommt sofort eine einzelne klare Fehlermeldung statt vieler verwirrender Einzelfehler pro Host.
  - **Unter Windows/IIS** (z.B. `C:\inetpub\wwwroot\...`): läuft PHP meist unter der App-Pool-Identität (z.B. `IIS APPPOOL\<Name-des-App-Pools>`) oder `IUSR`. Diese braucht **Schreibzugriff** auf **beide** Ordner:
    - `uploads\` (inkl. Unterordner `favicons\`) — hier landen die eigentlichen Icon-Dateien
    - `data\` — hier liegt `favicon_cache.json` (der Index, der Host → Icon-Datei zuordnet), ausserdem `links.json`, `config.json`, `audit_log.json`

    **Beide Ordner müssen separat berechtigt werden** — es reicht nicht, nur `uploads` freizugeben: die Icon-Dateien könnten dann zwar erfolgreich heruntergeladen werden, aber der Cache-Index in `data\favicon_cache.json` liesse sich nicht speichern, wodurch das Frontend die Dateien nie findet (Symptom: `uploads/favicons/` bleibt trotz erfolgreicher Downloads leer). Vorgehen: Rechtsklick auf `uploads` bzw. `data` → Eigenschaften → Sicherheit → Bearbeiten → passenden Benutzer hinzufügen (App-Pool-Identität als `IIS APPPOOL\<AppPoolName>` eintragen) → Schreiben/Ändern erlauben.
